  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const baseUrl = 'http://192.168.1.103:3000/api/v1';

        const urls = isCurrency
          ? [
              `${baseUrl}/currency/${symbol.slice(symbol.indexOf(':') + 1)}`,
              `${baseUrl}/currency/events/${symbol.slice(
                symbol.indexOf(':') + 1
              )}`,
              `${baseUrl}/currency/performance/${symbol.slice(
                symbol.indexOf(':') + 1
              )}`,
              `${baseUrl}/currency/news/${symbol.slice(
                symbol.indexOf(':') + 1
              )}/${lang}`,
            ]
          : [
              `${baseUrl}/stock/${marketCountry}/${symbol.slice(
                symbol.indexOf(':') + 1
              )}`,
              `${baseUrl}/stock/news?stockName=${symbol}`,
              `${baseUrl}/stock/performance?stockName=${symbol}`,
            ];
          ...
          <View style={styles.dataItem}>
            {data[1].data.slice(0, visibleItemsCount).map((event, index) => (
              <View key={index}>
                ...
                {event.urgency && (
                  <Text style={styles.dataText}>Urgency: {event.urgency}</Text>
                )}
                {event.published && (
                  <Text style={styles.dataText}>
                    Date:{' '}
